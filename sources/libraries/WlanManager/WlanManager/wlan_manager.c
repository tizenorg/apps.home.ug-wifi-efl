...
static void wlan_manager_register_cbs(void)
{
	...
	wifi_set_background_scan_cb(wlan_manager_network_event_cb, &scan_req_data);
...
static bool wifi_found_ap_with_state_cb(wifi_ap_h ap, void* user_data)
{
	...
	case WLAN_MANAGER_UNCONNECTED:
		if (WIFI_CONNECTION_STATE_DISCONNECTED == state ||
				WIFI_CONNECTION_STATE_FAILURE == state) {
...
wifi_connection_error_e wlan_manager_get_connection_error_state(wifi_ap_h ap)
{
	...
	if (!ap)
		return WIFI_CONNECTION_ERROR_UNKNOWN;

	profile_info = (net_profile_info_t *)ap;

	switch (profile_info->ProfileErrorState) {
	case NET_STATE_ERROR_NONE:
		return WIFI_CONNECTION_ERROR_NONE;
	case NET_STATE_ERROR_OUT_OF_RANGE:
		return WIFI_CONNECTION_ERROR_OUT_OF_RANGE;
	case NET_STATE_ERROR_PIN_MISSING:
		return WIFI_CONNECTION_ERROR_PIN_MISSING;
	case NET_STATE_ERROR_DHCP_FAILED:
		return WIFI_CONNECTION_ERROR_DHCP_FAILED;
	case NET_STATE_ERROR_CONNECT_FAILED:
		return WIFI_CONNECTION_ERROR_CONNECT_FAILED;
	case NET_STATE_ERROR_LOGIN_FAILED:
		return WIFI_CONNECTION_ERROR_LOGIN_FAILED;
	case NET_STATE_ERROR_AUTH_FAILED:
		return WIFI_CONNECTION_ERROR_AUTH_FAILED;
	case NET_STATE_ERROR_INVALID_KEY:
		return WIFI_CONNECTION_ERROR_INVALID_KEY;
	}
...
gboolean wlan_manager_is_same_network(wifi_ap_h ap1, wifi_ap_h ap2)
{
	...
	wifi_security_type_e ap1_sec, ap2_sec;

	if (!ap1 || !ap2) {
		return FALSE;
	}

	wifi_ap_get_security_type(ap1, &ap1_sec);
	wifi_ap_get_security_type(ap2, &ap2_sec);

	if (ap1_sec != ap2_sec) {
		return is_same;
	}

	wifi_ap_get_essid(ap1, &ap1_ssid);
	wifi_ap_get_essid(ap2, &ap2_ssid);
